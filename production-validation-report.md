...
| **BUG 5** | MEDIUM | Security Headers (`_headers`) | Configured Content-Security-Policy in `_headers` with `'unsafe-inline'` script-src to support 90+ inline UI event handlers across `index.html`, `js/tools.js`, and `js/ui.js`. Refactored boot and 404 configurations into dedicated external scripts (`js/boot-config.js` and `js/404-config.js`). |
